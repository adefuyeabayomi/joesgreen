...
function OrderItem({
  ...
  description = "",
  price,
}: FeaturedCardPropType): React.JSX.Element {
  return (
    <div className="text-main order-item-container-main">
      ...
      <div className="">
        ...
        <div className="order-description">
          <p className="font-small font-regular no-space text-main">
            {description}
          </p>
        </div>
        <div className="py-1"></div>
        <div>
          <p className="no-space font-subtitle font-regular text-main">
            N {price}
          </p>
        </div>
        <div className="py-1"></div>
        <div className="order-button-container text-main">
          <button className="pointer" onClick={actionFn}>
...
export function OrderCheckOutItem({ dish, onEdit, onRemove }: COIType) {
  function getAddonText(arr) {
    let res = arr
      .map((addon) => {
        ...
        return `${addon.quantity} ${addon.name}, `;
      })
      .join("");
    console.log({ res });
    return res;
  ...
  return (
    <div className="orderCheckOutItemContainer text-main">
      <div>
        <p className="font-subtitle font-bold no-space green-color-main">
          {dish.quantity} Plate{dish.quantity > 0 ? "s" : ""} Of {dish.name}
        </p>
      </div>
      <div className="py-1" />
      <div>
        {dish.addons.length > 0 && (
          <div>
            ...
            <p className="font-small font-regular no-space">
              {getAddonText(dish.addons).length == 0
                ? "No Addons Added"
                : getAddonText(dish.addons)}
            </p>
          </div>
        )}
      </div>
      ...
      <div>
        ...
        <div className="row no-space font-small font-regular">
          <div className="col no-space">
            <p className="no-space">
              <span>Addons Included Per Plate: </span>
            </p>
          </div>
          <div className="w-max-content no-space">
            {" "}
            N: {calculateAddonsTotalPrice(dish.addons)}
          </div>
        </div>
        ...
        <div className="row no-space font-small font-regular">
          ...
          <div className="w-max-content no-space">
            N{" "}
            {calulateTotalOrderPrice(
              dish.price,
              calculateAddonsTotalPrice(dish.addons),
              dish.quantity,
            )}
          </div>
        </div>
      ...
      <div className="row no-space">
        <div className="col no-space">
          <button
            className="OCI_Button BEdit font-small font-regular"
            onClick={onEdit}
          >
            Edit Order
          </button>
        ...
        <div className="w-max-content no-space">
          <button
            className="OCI_Button BRemove font-small font-regular"
            onClick={() => {
              onRemove(dish.id);
            }}
          >
...
export function UserOrderItem({ dish, payFn }: COIType) {
  ...
  return (
    <div className="orderCheckOutItemContainer text-main">
      <div>
        <p className="font-subtitle font-bold no-space">
          {dish.quantity} Plate{dish.quantity > 0 ? "s" : ""} Of {dish.name}
        </p>
      </div>
      <div>
