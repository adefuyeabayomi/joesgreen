...
export default function Login(): React.JSX.Element {
  const { triggerInfo, triggerError } = useNotificationTrigger();
  ...
  function goToSignup() {
    if (proceedTo) {
      navigate(`/signup?proceedTo=${proceedTo}`);
    } else {
      navigate(`/signup`);
    }
  }
  ...
  async function sendLogin() {
    ...
    try {
      ...
      triggerInfo({
        title: "Login Success",
        message:
          "Your Login is successful. You can now use the full functions of the site",
      });
      navigate(`/${proceedTo}` || "/");
    } catch (error) {
          ...
          <div className="row no-space">
            <div className="col-12 col-sm-6 no-space">
              <div className="loginFormContainer">
                ...
                <div className="py-2" />
                <div>
                  <p className="font-small text-center no-space">
                    Forgot Password?{" "}
                    <span
                      className="font-regular pointer"
                      onClick={goToForgotPassword}
                    >
                      Click to recover it now.
                    </span>{" "}
                  </p>
                </div>
                <div className="py-2" />
