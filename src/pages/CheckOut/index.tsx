...
export default function CheckOut(): React.JSX.Element {
  ...
  let cartItems = useSelector(
    (state: { user: { userCart } }) => state.user.userCart,
  );
  const dispatch = useDispatch();
  const { setLoading, setLoadingText } = useLoading();
  const { triggerError, triggerInfo, triggerSuccess } =
  ...
  const handleCheckout = async () => {
    ...
    if (!auth.isAuthenticated) {
      triggerInfo({
        ...
        message:
          "You need to login to be able to track your order on the website.",
      });
      setTimeout(() => {
        navigate(`/login?proceedTo=checkout`);
      }, 1000);
      return;
    }
    ...
    const order = {
      ...
      narration: cartItems
        .map(
          (item) =>
            `${item.quantity} plate${item.quantity > 1 ? "s" : ""} of ${item.name}`,
        )
        .join(", "),
        ...
        <CustomDialog isOpen={showEdit} onClose={toggleEdit}>
          <div>
            <p className="font-subtitle font-medium">
              {" "}
              Include Addons to Plate
            </p>
            <div>
              {currentDish
                ? currentDish.addons.map((x) => {
                    return (
                      <div key={x._id} className="row">
                        <div className="col">
                          <p>
                            <span> {`${x.name}: (N ${x.price}/unit), `} </span>
                          </p>
                        </div>
                        <div className="w-max-content">
                          <p>
                            {" "}
                            <button
                              className="font-p font-medium text-center addonOpButton"
                              onClick={() => increaseAddonQuantity(x._id)}
                            >
                              +
                            </button>{" "}
                            {x.quantity}{" "}
                            <button
                              className="font-p font-medium text-center addonOpButton"
                              onClick={() => decreaseAddonQuantity(x._id)}
                            >
                              -
                            </button>
                          </p>
            ...
            <div className="py-1" />
            <div>
              <button className="UpdateAddonButton" onClick={handleAddToCart}>
                Update
              </button>
            </div>
          </div>
        ...
        <div className="container-fluid no-space">
          <div className="row no-space">
            <div className="col-12 no-space">
              <div>
                <p className="font-heading-6 green-color-main font-medium">
                  PROCEED TO CHECKOUT
                </p>
                <p className="font-p col-12 col-md-9">
                  Check out your cart items. You can edit your meals with our
                  addons. Just click the edit button.
                </p>
              </div>
              <div className="row no-space">
                {cartItems.length > 0 ? (
                  cartItems.map((item, index) => {
                    return (
                      <div
                        className={`no-space col-12 col-md-6 ${index % 2 !== 0 ? "checkout-margin-left" : null}`}
                      >
                        <div className="py-2" />
                        <OrderCheckOutItem
                          key={item._id}
                ...
                <div className="orderContainer">
                  ...
                  <div className="summaryItem">
                    ...
                    <div className="deliveryInformation">
                      ...
                      <div className="py-1" />
                      <div className="">
                        <label className="font-p font-regular">
                          Phone Number
                        </label>
                        <div className="py-1" />
                        <InputMain
                          onFocus={() => {
                            resetInputError("phoneNumber");
                          }}
                          showError={inputError.phoneNumber}
                          errorMessage="Phone Number is required"
                          type="number"
                          value={phoneNumber}
                          onChange={(e) => setPhoneNumber(Number(e))}
                          placeholder="Phone Number"
                          icon={
                            <FontAwesomeIcon color="#737373" icon={faPhone} />
                          }
                        />
                      </div>
                      <div className="py-1" />
                      <div className="">
                        ...
                        <InputMain
                          onFocus={() => {
                            resetInputError("deliveryInfo");
                          }}
                          showError={inputError.deliveryInfo}
                          value={deliveryInfo}
